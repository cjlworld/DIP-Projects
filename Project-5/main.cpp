...
int main(int argc, char *args[])
{
    ...
    string inStr(args[1]), outStr(args[2]), transStr(args[3]);
    // cerr << inStr << " " << outStr << endl;

    bmp.readFile(inStr);
    colMap.readBMP(bmp);

    // 转换为 YUV 图
    YUVColorMap gcolMap = colMap.toYUVColorSpace();

    if(transStr == "mean") {
    	gcolMap = gcolMap.meanFilter();
    } else if(transStr == "laplacian") {
    	gcolMap = gcolMap.laplacianEnhancement();
    } else {
    	cerr << "NO THIS OPERATION: " << transStr << endl;
    }
    gcolMap.toRGBColorSpace().toBMP().writeFile(outStr);
    return 0;
}
