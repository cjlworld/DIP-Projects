...
int main(int argc, char *args[])
{
    ...
    string inStr(args[1]), outStr(args[2]), transStr(args[3]), rbfStr(args[4]);
    ...
    RBFunc phi = nullptr;
    TransFunc trans = nullptr;

    if(rbfStr == "gaussian") {
    	phi = GeometricGraph<RGBPixel>::gaussianRadialBasisFunc;
    } else if(rbfStr == "cubic") {
    	phi = GeometricGraph<RGBPixel>::cubicRadialBasisFunc;
    } else if(rbfStr == "thinplate") {
    	phi = GeometricGraph<RGBPixel>::thinplateRadialBasisFunc;
    } else {
    	cerr << "No RBF: " << rbfStr << endl;
    	return 0;
    }

    string arg1(args[5]);
    if(transStr == "rotate") {
    	std::stringstream ssin(arg1);
    	double theta;
    	ssin >> theta;
    	trans = GeometricGraph<RGBPixel>::Rotate(theta);
    } else {
    	string arg2(args[6]);

    	double c, d;
    	std::stringstream ssin_1(arg1);
    	ssin_1 >> c;
    	std::stringstream ssin_2(arg2);
    	ssin_2 >> d;

    	if(transStr == "mirror") {
    		trans = GeometricGraph<RGBPixel>::Mirror(c, d);
    	} else if(transStr == "scale") {
    		trans = GeometricGraph<RGBPixel>::Scale(c, d);
    	} else if(transStr == "translation") {
    		trans = GeometricGraph<RGBPixel>::Translation(c, d);
    	} else if(transStr == "shear") {
    		trans = GeometricGraph<RGBPixel>::Shear(c, d);
    	} else {
    		cerr << "No transform: " << transStr << endl;
    		return 0;
    	}
    }

    colMap.geometricTransform(trans, phi).toBMP().writeFile(outStr);
/*
    // 平移
    colMap.geometricTransform(GeometricGraph<RGBPixel>::Translation(-30, 0), GeometricGraph<RGBPixel>::gaussianRadialBasisFunc).toBMP().writeFile("TEST-0-O-Translation.bmp");

    // 旋转
    colMap.geometricTransform(GeometricGraph<RGBPixel>::Rotate(PI/4), GeometricGraph<RGBPixel>::gaussianRadialBasisFunc).toBMP().writeFile("TEST-0-O-Rotate.bmp");

    // 缩放
    colMap.geometricTransform(GeometricGraph<RGBPixel>::Scale(3, 1), GeometricGraph<RGBPixel>::cubicRadialBasisFunc).toBMP().writeFile("TEST-0-O-Scale.bmp");

    // 错切
    colMap.geometricTransform(GeometricGraph<RGBPixel>::Shear(3, 0), GeometricGraph<RGBPixel>::cubicRadialBasisFunc).toBMP().writeFile("TEST-0-O-Shear.bmp");

    // 镜像
    colMap.geometricTransform(GeometricGraph<RGBPixel>::Mirror(-1, 1), GeometricGraph<RGBPixel>::cubicRadialBasisFunc).toBMP().writeFile("TEST-0-O-Mirror.bmp");
*/
    return 0;
}
